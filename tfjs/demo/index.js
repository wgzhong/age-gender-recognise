...
const IMAGENET_CLASSES = ["F", "less18", "18-60", "over60"]
...
async function predict(imgElement) {
  ...
  const logits = tf.tidy(() => {
    ...
    const batched = normalized.reshape([1, IMAGE_SIZE_H, IMAGE_SIZE_W, 3]);
...
filesElement.addEventListener('change', evt => {
  ...
  for (let i = 0, f; f = files[i]; i++) {
    ...
    reader.onload = e => {
      ...
      img.width = IMAGE_SIZE_W;
      img.height = IMAGE_SIZE_H;
